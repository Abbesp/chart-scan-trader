...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "get_market_data") {
      try {
        ...
        const tickerResponse = await fetch(
          `${baseUrl}/api/v1/market/allTickers`
        );
    ...
    if (action === "get_kline_data" && symbol && interval) {
      const klineUrl = `${baseUrl}/api/v1/market/candles?symbol=${symbol}&type=${interval}&startAt=${
    ...
    if (action === "place_order" && orderData) {
      ...
      return new Response(
        JSON.stringify({ success: true, ...orderResult }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ errorMessage: "Invalid action" }),
